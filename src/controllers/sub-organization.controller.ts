...
export class SubOrganizationController {
    ...
    static async getAll(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            res.status(200).json({ status: 'success', data: { subOrganizations } });
        } catch (error) {
            next(error);
        }
    }
    ...
    static async create(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            res.status(201).json({ status: 'success', data: { subOrganization } });
        } catch (error) {
            next(error);
        }
    }
    ...
    static async update(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!subOrganization) return next(new AppError('SubOrganization not found', 404));

            repo.merge(subOrganization, req.body);
            const result = await repo.save(subOrganization);
            res.status(200).json({ status: 'success', data: { subOrganization: result } });
        } catch (error) {
            next(error);
        }
    }
    ...
    static async delete(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (result.affected === 0) return next(new AppError('SubOrganization not found', 404));
